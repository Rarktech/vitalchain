...
function buildLocalEntity({ entityKey, txHash, entityType, evmAddress, suiOwner, createdAt, expiresInDays, attributes, payload }) {
  const now = createdAt || Date.now();
  return {
    entityKey,
    entityType,
    txHash,
    $owner: suiOwner || evmAddress,
    $creator: evmAddress,
    createdAt: now,
...
export function VCProvider({ children }) {
  ...
  const writeAnalysis = async ({ question, analysis, trend, recommendations, readingKeys, model }) => {
    ...
    const attrs = [
      ...
      { key: 'model', value: model || 'gemini-2.5-flash' },
