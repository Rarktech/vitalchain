...
export function VCProvider({ children }) {
  ...
  const connectWallet = async (overrideAddress) => {
    ...
    setState(s => ({
      ...s,
      wallet: { address, balance: '0.482 SUI', connectedAt: Date.now() },
      entities: {},
      activity: [],
    }));
    pushToast({ title: 'Wallet connected', body: 'Sui Testnet · zkLogin' });
    return address;
  };
  ...
  const addReading = async ({ deviceKey, readingType, primaryValue, secondaryValue, note, encrypt }) => {
    ...
    const payloadObj = { value: primaryValue, secondaryValue, note: note || '', deviceId: deviceKey };
    let payload;
    if (encrypt) {
  ...
  const seedDemo = async (walletAddressOverride) => {
    ...
    const mkReading = (device, type, p1, p2, daysAgo) => {
      ...
      const ent = {
        ...
        payload: { value: p1, secondaryValue: p2, note: '', deviceId: device.key },
      };
      entities[key] = ent;
    };
