...
import {
  arkivCreate, arkivCreateWithDevicePvk, arkivDelete, arkivLoadAll,
  getArkivAddress, getOrCreateDeviceKey, getDeviceAddress,
} from './arkiv';
...
export function VCProvider({ children }) {
  ...
  const addReading = async ({ deviceKey, readingType, primaryValue, secondaryValue, note, encrypt }) => {
    ...
    // Try signing with device keypair so $creator = device address
    const devicePvk = await getOrCreateDeviceKey(deviceKey);
    const userEvmAddress = await getArkivAddress(state.wallet.address);

    const result = await arkivCreateWithDevicePvk(devicePvk, userEvmAddress, {
  ...
  const registerDevice = async ({ name, model, deviceType, firmware }) => {
    ...
    // Generate device keypair and store it; deviceId = device EVM address
    const devicePvk = await getOrCreateDeviceKey(result.entityKey);
    const deviceAddress = await getDeviceAddress(result.entityKey);

    // Update entity on chain to include deviceId attribute (or just track locally)
    const allAttrs = [
      { key: 'deviceId', value: deviceAddress },
      ...attrs,
    ];
    ...
    const entity = buildLocalEntity({
      ...
      expiresInDays: 365,
      attributes: allAttrs,
      payload: { name, model, firmwareVersion: firmware || '1.0.0', deviceAddress },
    });
